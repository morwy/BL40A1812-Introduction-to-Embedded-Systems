...
#define ADMUX_MUX_SINGLE_A3 (0b00011)
...
static void select_input_channel(uint8_t adc_mux) {
    ...
    SET_BITS(ADC_CONTROL_STATUS_B, ADCSRB_MUX_OFFSET, (adc_mux >> 3) & ADCSRB_MUX_MASK);
}
...
void setup_adc(void) {
    reset_adc_module();
    select_input_channel(ADMUX_MUX_SINGLE_A3);
    ...
    printf("ADC Multiplexer: %x\n", ADC_MULTIPLEXER);
    printf("ADC Control Status A: %x\n", ADC_CONTROL_STATUS_A);
    printf("ADC Control Status B: %x\n", ADC_CONTROL_STATUS_B);
}